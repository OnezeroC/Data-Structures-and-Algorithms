...
protected:
    ...
    struct BinaryNode
    {
         ...
         * @param h 节点的初始高度
         */
        BinaryNode(const Comparable &theElement, BinaryNode *lt = nullptr, BinaryNode *rt = nullptr, int h = 0)
         ...
         * @param h 节点的初始高度
         */
        BinaryNode(Comparable &&theElement, BinaryNode *lt = nullptr, BinaryNode *rt = nullptr, int h = 0)
            : element{ std::move(theElement) }, left{ lt }, right{ rt }, height{ std::move(h) } {}
    };
    ...
    void balance(BinaryNode*& t) {
        // If the node is null, no balancing is needed
        if (t == nullptr) return;

        // Check if the left subtree is too tall (balance factor > 1)
        if (height(t->left) - height(t->right) > 1) {
            // Perform a single rotation with left child if the left-left case
            if (height(t->left->left) >= height(t->left->right))
                rotateWithLeftChild(t);
            // Otherwise, perform a double rotation (left-right case)
            else
                doubleWithLeftChild(t);
        }
        // Check if the right subtree is too tall (balance factor < -1)
        else if (height(t->right) - height(t->left) > 1) {
            // Perform a single rotation with right child if the right-right case
            if (height(t->right->right) >= height(t->right->left))
                rotateWithRightChild(t);
            // Otherwise, perform a double rotation (right-left case)
            else
    ...
    // Single rotation with left child for left-left imbalance
    void rotateWithLeftChild(BinaryNode*& k2) {
        BinaryNode* k1 = k2->left;
        k2->left = k1->right;     // Move k1's right subtree to k2's left
        ...
        k2 = k1;                  // Update k2 to point to the new root of this subtree
    }

    // Single rotation with right child for right-right imbalance
    void rotateWithRightChild(BinaryNode*& k1) {
        BinaryNode* k2 = k1->right;
        k1->right = k2->left;     // Move k2's left subtree to k1's right
        k2->left = k1;            // Set k1 as the left child of k2

        // Update heights after rotation
        k1->height = std::max(height(k1->left), height(k1->right)) + 1;
        k2->height = std::max(height(k2->right), k1->height) + 1;

        k1 = k2;                  // Update k1 to point to the new root of this subtree
    }

    // Double rotation for left-right imbalance
    void doubleWithLeftChild(BinaryNode*& k3) {
        rotateWithRightChild(k3->left);  // First rotate the left child to the right
        rotateWithLeftChild(k3);         // Then perform a left rotation
    }

    // Double rotation for right-left imbalance
    void doubleWithRightChild(BinaryNode*& k1) {
        rotateWithLeftChild(k1->right);  // First rotate the right child to the left
        rotateWithRightChild(k1);        // Then perform a right rotation
    }
    ...
    void remove(const Comparable &x, BinaryNode *&t) {
        ...
        if (x < t->element) {
            remove(x, t->left); // 在左子树中查找
        } else if (x > t->element) {
            remove(x, t->right); // 在右子树中查找
        } else {
            // 找到了要删除的节点
            if (t->left != nullptr && t->right != nullptr) { // 有两个子节点
                // 使用 detachMin 函数替代节点内容复制
                ...
                t = (t->left != nullptr) ? t->left : t->right; // 替换节点
